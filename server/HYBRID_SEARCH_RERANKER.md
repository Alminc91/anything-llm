...
| `hybrid_rerank` **(recommended)** | The arms share the `reranker_retrieval_topk` nomination budget via per-arm quotas (`hybrid_arm_split`, default 50/50; shares sum to the budget exactly). Slots freed by overlap or an under-delivering/failed arm are backfilled from the other arm, so the reranker always receives min(budget, available) candidates — none is dropped by RRF before the reranker has judged it. RRF order is used only as a fallback if the reranker fails; `hybrid_weight` has no effect on which candidates the reranker sees. | Yes |
...
| `reranker_retrieval_topk` | `40` | `1`–`500` | **Total** documents sent to the reranker. In `hybrid_rerank` the arms share this nomination budget via quotas (see `hybrid_arm_split`); unused slots are backfilled so the budget is always exhausted when candidates exist. Also bounds the native CPU reranker's workload. |
| `hybrid_arm_split` | `0.5` | `0.1`–`0.9` | Vector share of the `hybrid_rerank` nomination budget (Qdrant-prefetch-style per-arm quota; BM25 gets the rest). `0.5` = equal halves; raise for semantics-heavy corpora, lower for keyword-heavy ones. Does **not** weight results — the reranker judges all nominees purely by relevance. |
