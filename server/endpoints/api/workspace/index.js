...
function apiWorkspaceEndpoints(app) {
  ...
  app.post(
    ...
    */
      try {
        ...
        console.log(
          `Attempting to update workspace ${currWorkspace.id} with data:`,
        ...
        try {
          // Wrap the update in its own try block for specific logging
          ...
          // ADD SUCCESS LOG:
          console.log(
            `Successfully updated workspace ${currWorkspace.id}. New limit: ${workspace.messagesLimit}`
          );
          response.status(200).json({ workspace, message });
        } catch (updateError) {
          // ADD ERROR LOG:
          console.error(
            `Failed to update workspace ${currWorkspace.id}:`,
            updateError.message,
            updateError
          );
   ...
   */
      try {
        ...
        const { contingent, messagesLimit } =
